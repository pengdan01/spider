...
using crawler2::crawl_queue::FeatureItem;
...
class CrawlTaskHandler {
 public:
  ...
  virtual void Process(CrawlTask* task,
                       CrawlRoutineStatus* status, crawler2::Resource* res,
                       Closure* done) = 0;
};
...
class GeneralCrawler {
 public:
  ...
  struct JobData {
    ...
    int32_t from_partition; // 来自于哪个partition
    JobData() : status(0), done(NULL), from_partition(100000) {
    }
 ...
 private:
  // 根据 Jobetail 中的 key-values 设置 抓取 task 的相关参数
  void HandleJobDetailFeatures(const JobDetail &detail, CrawlTask *task);
