...
DEFINE_int64_counter(general_crawler, timedout_tasks, 0, "在抓取队列当中存在过长且没有得到抓取机会的任务");
DEFINE_int64_counter(general_crawler, task_checked, 0, "检查 task 是否符合 load control 的");
DEFINE_int64_counter(general_crawler, fetching_task, 0, "正在抓取的任务数");
DEFINE_int64_counter(general_crawler, task_completed, 0, "抓取完成的个数");
DEFINE_int64_counter(general_crawler, curl_error, 0, "出现 curl 错误的任务数");
DEFINE_int64_counter(general_crawler, http_2xx, 0, "返回 HTTP 2XX 的任务数");
DEFINE_int64_counter(general_crawler, http_4xx, 0, "返回 HTTP 4XX 的任务数");
DEFINE_int64_counter(general_crawler, http_5xx, 0, "返回 HTTP 5XX 的任务数");
...
void GeneralCrawler::FetchTaskDone(crawler2::crawl::CrawlTask* task,
  ...
  if (!status->success) {
    ...
    COUNTERS_general_crawler__curl_error.Increase(1);
  } else if (status->ret_code / 100 == 4) {
    stat_.total_http_4xx++;
    COUNTERS_general_crawler__http_4xx.Increase(1);
  } else if (status->ret_code / 100 == 5) {
    stat_.total_http_5xx++;
    COUNTERS_general_crawler__http_5xx.Increase(1);
  } else if (status->ret_code / 100 == 2) {
    stat_.total_http_2xx++;
    COUNTERS_general_crawler__http_2xx.Increase(1);
  }
...
void GeneralCrawler::Process(crawler2::crawl::CrawlTask* task, crawler2::crawl::CrawlRoutineStatus* status,
                              crawler2::Resource* res) {
  JobData* data = reinterpret_cast<JobData*>(task->extra);
  ...
  COUNTERS_general_crawler__task_completed.Increase(1);
...
int GeneralCrawler::CheckTasks(std::deque<crawler2::crawl::CrawlTask*>* tasks) {
  ...
  for (int i = 0; i < task_size; ++i) {
    ...
    COUNTERS_general_crawler__task_checked.Increase(1);
    ...
    if (ret == 0) {
      ...
      if (current_timestamp - extra->timestamp
        ...
        COUNTERS_general_crawler__timedout_tasks.Increase(1);
...
void GeneralCrawler::AddTask(JobData* data) {
  ...
  if (hosts_use_proxy_.find(gurl.host()) != hosts_use_proxy_.end()) {
