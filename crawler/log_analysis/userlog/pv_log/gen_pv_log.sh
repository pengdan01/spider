...
DEFINE_string  'input_path'   '/data/userlog/token_url'         'input hdfs path'      'i'
...
### Round 1, convert referer md5 to referer url
file="pv_r1.opt"
mapper_cmd="./$file --md5_url_path `dirname ${md5_url_path}` --pv_log_path `dirname ${raw_input}` --hot_md5_path hot_md5"
reducer_cmd="./$file --md5_url_path `dirname ${md5_url_path}` --pv_log_path `dirname ${raw_input}` --hot_md5_path hot_md5"
...
./mapreduce/submit.py --mr_map_cmd $mapper_cmd --mr_reduce_cmd $reducer_cmd \
    ...
    --mr_reduce_tasks 5000 --mr_files hot_md5 \
    --mr_slow_start_ratio 0.95 --mr_multi_output \
...
### Round 3, sort first by mid, second by time_stamp
