...
DEFINE_string  'output_pv_path'  "/tmp/$USER/$$.`date +%d%H%M%S`"  'output hdfs path'     ''
...
${HADOOP_HOME}/bin/hadoop fs -test -d ${FLAGS_output_pv_path}
[[ $? -ne 0 ]] && ${HADOOP_HOME}/bin/hadoop fs -mkdir ${FLAGS_output_pv_path}
...
final_tasks=$(echo $(HadoopDus $raw_input) | awk '{print int($0/8000000000)+1}')

mapper_cmd="./pv_plus.opt"
reducer_cmd="./pv_plus.opt"
...
./mapreduce/submit.py \
  ...
  --mr_job_name new_pv_log.$USER.`date +%d%H%M` \
  --mr_disable_input_suffix_check \
  --mr_max_map_failures_percent 1 \
...
# output pv log
final_output="${FLAGS_output_pv_path}/${FLAGS_date}_00_00.st" 
$HADOOP_HOME/bin/hadoop fs -rmr $final_output
$HADOOP_HOME/bin/hadoop fs -mkdir $final_output
$HADOOP_HOME/bin/hadoop fs -mv $output/pvlog* $final_output
...
$HADOOP_HOME/bin/hadoop fs -rm $final_output/title_pv*
$HADOOP_HOME/bin/hadoop fs -mv $output/title_pv* $final_output
[[ $? -ne 0 ]] && SendAlertViaMailAndDie "hadoop mv failed" "${FLAGS_alert_mail}"
