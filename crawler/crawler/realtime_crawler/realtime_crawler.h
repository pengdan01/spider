...
class CrawlTaskHandler {
 public:
  ...
  virtual void Process(crawl::CrawlTask* task,
                       crawl::CrawlRoutineStatus* status, Resource* res,
                       Closure* done) = 0;
};
...
class RealtimeCrawler {
 ...
 private:
  ...
  struct TaskStatus {
    Resource* res;
    crawl::CrawlTask* task;
    crawl::CrawlRoutineStatus* crawl_status;
  ...
  int CheckTasks(std::deque<crawl::CrawlTask*>* tasks);
  void CheckQueue();
  void Process(::crawler2::crawl::CrawlTask* task,
               ::crawler2::crawl::CrawlRoutineStatus* status, Resource* res);
  ...
  // 变量名字取得不太友好, 这个是待抓取的任务, 而 不 是已经抓取
  ...
  crawl::LoadController load_controller_;
  crawl::ProxyManager proxy_manager_;
  crawl::PageCrawler page_crawler_;
