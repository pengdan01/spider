...
#target_ip=180.153.227.166
target_ip=10.115.88.12
target_port=34000

last_counter_file=data/m11_realtime_crawler_counter

counter_file=http://${target_ip}:${target_port}/counters

local_counter_file=.success_rate_counter_file

rm -f ${local_counter_file}

wget ${counter_file} -O ${local_counter_file}
if [ $? -ne 0 ]; then
  err_msg="realtime_crawler:crawl_success_ratio_monitor: fail in wget ${counter_file} -O ${local_counter_file}"
...
if [ $delta_task_completed -eq 0 ]; then
  err_msg=`realtime_crawler:crawl_success_ratio_monitor: warning, crawl 0 urls since $last_timestamp`
  SendAlertViaMail "$err_msg" "$receiver" 
else
  ...
  msg="realtime_crawler[crawl_success_ratio_monitor]: curl_ok_ratio: ${c_ok}k/${total}k=$curl_ok_ratio, http_2xx_ratio: ${http_ok}k/${total}k=$http_2xx_ratio, since $last_timestamp"
