...
void JobManager::PutJobProc() {
  ...
  RealtimeCrawler::JobData* job = NULL;
  while ((job = jobs_to_put_.Take()) != NULL) {
    serialized.clear();
    scoped_ptr<RealtimeCrawler::JobData> auto_job(job);
...
void JobManager::CheckJobQueue() {
  ...
  while (!jobs_to_put_.Closed()) {
    ...
    while (job_fetched_.Size() < num_to_fetch_) {
      ...
      task_count++;
      RealtimeCrawler::JobData* data = new RealtimeCrawler::JobData;
...
void JobManager::JobDone(int64 jobid, RealtimeCrawler::JobData* job) {
  if (job->status == 0) {
    task_done_++;
    delete job;
  } else {
