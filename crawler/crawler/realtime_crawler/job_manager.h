...
class JobManager {
 public:
  ...
  typedef std::deque<RealtimeCrawler::JobData*> JobQueue;
 ...
 private:
  ...
  void JobDone(int64 jobid, RealtimeCrawler::JobData* job);
  ...
  ::thread::BlockingQueue<RealtimeCrawler::JobData*> jobs_to_put_;
  // 存放从 job queue 中获取到的任务的 blocking queue
  ::thread::BlockingQueue<RealtimeCrawler::JobData*> job_fetched_;
