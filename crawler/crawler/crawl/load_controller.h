...
class LoadController {
 public:
  struct Options {
    ...
    // 累计抓取次数没达到 check_frequency_ 时，不做 QPS 检查
    int check_frequency;
    ...
    // Add by pengdan(20140825)
    std::string ip_load_file;
  ...
  struct IpLoadRecord {
    ...
    // 发起 check_frequency 次抓起应该耗费的时间
    // 由 check_frequency 和 max_qps 计算得到, 不在配置文件中存储
    int64 min_duration;
 ...
 private:
  ...
  struct IpLoadStatus {
    ...
    // NOTE:
    // 每次 fetch_times 刚好超过 check_frequency 时,
