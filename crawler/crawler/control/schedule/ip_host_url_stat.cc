...
void Reducer() {
  ...
  while (task->NextKey(&key)) {
    ...
    for (size_t host_i = 0; host_i < host_order.size(); ++host_i) {
      ...
      for (size_t url_i = 0; url_i < 3 && url_i < host_url[host_order[host_i]].size(); ++url_i) {
        output_value += host_url[host_order[host_i]][url_i] + "\t";
      }
    }
    // auto it = host_count.begin();
    // for ( ; it != host_count.end(); ++it) {
    //   output_value += it->first + "\t";       // host
    //   output_value += base::Int64ToString(it->second) + "\t"; // host 对应的 url 个数
    //   for (size_t url_i = 0; url_i < 3 && url_i < host_url[it->first].size(); ++url_i) {
    //     output_value += host_url[it->first][url_i] + "\t";
    //   }
    // }
