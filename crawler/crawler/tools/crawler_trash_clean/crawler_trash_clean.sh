...
if [ $# -ne 1 ]; then
  echo "Error, Usage: $0 crawler_trash_clean.conf"
  exit 1
fi
conf_file=$1
if [ ! -e ${conf_file} ]; then
...
do
  source_type=$i
  if [ $source_type != "page" -a $source_type != "css" -a $source_type != "image" ]; then
    echo "Error, Invalid source type: ${source_type}, shoud be page | css | image"
    exit 1
  fi
  if [ $source_type = "page" ]; then
