...
batch_hdfs_dir=${batch_hdfs_root_dir}/shard_${format_shard_id}
delta_hdfs_dir=${delta_hdfs_root_dir}/shard_${format_shard_id}

trash_batch_dir=${trash_hdfs_root_dir}/batch/${merge_type}/shard_${format_shard_id}
trash_delta_dir=${trash_hdfs_root_dir}/delta/${merge_type}/shard_${format_shard_id}
...
build_tools/mapreduce/submit.py \
  ...
  --mr_job_name "crawler_combine_${merge_type}.${format_shard_id}"
if [ $? -ne 0 ]; then
...
## XXX(pengdan): 考虑到时效性需要用到每天增量的 网页, 这里不删除
