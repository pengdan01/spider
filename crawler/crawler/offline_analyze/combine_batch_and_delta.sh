...
## 将增量目录某个 shard 的 page / css / image 等文件和对应的 batch 目录下的
## page / css / image 进行 merge
if [ $# -ne 1 ]; then
  echo "Error, Usage: $0 combine_batch_and_delta.conf"
  exit 1
fi
conf_file=$1
if [ ! -e ${conf_file} ]; then
...
source ${conf_file}
status_dir=~/crawler_status/combine_batch_delta
[ ! -d ${status_dir} ] && mkdir -p ${status_dir}

if [ ! "${shard_num_info_hdfs_file}" -o ! "${shard_id}" -o ! "${merge_type}" ]; then
  echo "Error, var shard_num_info_hdfs_file/shard_id/merge_type is empty"
  exit 1
fi
...
## 获取 shard num
shard_num=`${HADOOP_HOME}/bin/hadoop fs -cat ${shard_num_info_hdfs_file} | grep "^${merge_type}_shard_num=" \
| awk -F'=' '{print $2}'`
if [ ! "${shard_num}" ]; then
...
do
  shard_id=$i
  source crawler/offline_analyze/combine_batch_and_delta_internal.sh
done
